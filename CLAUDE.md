...
- **[항목 1] 국내 수집 대폭 확대** (`news.py`·`news_sources.py`, 커밋됨·**미배포**): 네이버 `display` 10→100,
  ...
  요청 간격 0.35s. 실측 국내 84→2,357건. **효과는 `railway up` 후 크론부터.**
- **[항목 2] 분류 프롬프트 v3** (`news_classify.py`, 커밋됨·미배포): 10대 분야에 **정부 공식 정의 삽입**
  (`data/푸드테크_10대분야.pdf`) + "기본값 해당없음" 빡센 필터 + 키워드 함정 차단. 재채점 16/16, 캐시 80건
  폐기 43% 전건 정확. **입력은 제목+요약 300자 유지**(본문 스크래핑 안 함 — 파일럿 확정, 결정 2).
- **2차 관련성 게이트**(`filter_foodtech_relevant`) 유지 = 발송 직전 LLM. 1차(수집)+2차(발송) 둘 다 씀.
  ⚠️ 게이트는 아직 **앱 발송 경로에 미배선**(scratchpad 조립에서만). 발송 자동화 때 배선 필요.
- **pilot_members 테이블**(신규, RLS) — 랩실 25명 프로필+발송/추적 집계+Activity Score 컬럼. `refresh_pilot_stats`
  자동화 전이라 갱신은 백필 스크립트 수동. **파일럿 정식 발송 #906**(전원 동일 1편, 3:2+게이트) 25명 실발송·추적 관통.
- **남은 로드맵(순서)**: 3 파일럿 매일발송 자동화(`pilot_daily.py`+`/jobs`+크론, scratchpad→앱 승격) →
  4 관리자 페이지(탭: 회원관리·인기분야·세그먼트 스코어·발송 전 검토) → 5 뉴스레터 디자인(Canva) →
  Activity Score 함수. 파일럿 발송·3:2 조립은 현재 scratchpad 스크립트로만 돎.
