...
- ⚠️ **LLM 판정은 배치 크기가 정한다.** 게이트가 풀 전체(117건)를 한 번에 던져 **drop 0**이
  됐고, 심도가 평평해지자 T-024의 "심도로 메인 선정"이 통째로 무효가 됐다 — 아파트 분양
  기사가 #026 메일 제목이 됐다. **`BATCH_SIZE`(20)로 쪼개면 drop 31.** 프롬프트는 멀쩡했다.
  판정 도구를 의심할 땐 프롬프트보다 **한 호출에 몇 건을 넣었나**를 먼저 본다.
