...
1. ~~T-001 뉴스 수집~~ ✅ + ~~T-006 분류 통합·DB 이관~~ ✅ 2026-07-18 — 수집(네이버+Brave→RSS 폴백)
   → LLM 분류(gemini-2.5-flash, 신규 URL만) → **`news_items` 테이블 저장**(영문 슬러그 11종 =
   정부 10대 + general, "해당없음" 폐기). 파일 캐시는 헬스체크용 유지. 학술(OpenAlex)은 별도 티켓.
