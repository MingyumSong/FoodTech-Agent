...
| 축 | 값 | 쓸모 |
| --- | --- | --- |
| `group_no` | 1~5 (각 5명) | **A/B 그룹** — 원래 용도였던 T-016은 접혔지만(2026-08-18), 어떤 실험을 하든 배정 축으로 그대로 쓴다 |
...
2. 운영 데이터 렌더 + 헤드리스 크롬 스크린샷 검수 완료. 실제 산출:
   ...
   - 발송 그룹: 1조 24.6 > 4조 23.2 > 5조 17.4 > 3조 11.1 > 2조 10.6 (각 5명).
     **그룹 간 격차가 2배 이상** — 이 그룹으로 A/B를 돌릴 때 대조군을 그냥 고르면 안 된다는
     신호다(사전 참여도가 다르므로 짝을 맞춰야 한다). T-016은 접혔지만 **이 경고는 실험 일반에
     유효하다** — 조 단위로 배정하지 말고 점수 순위로 층화할 것.
