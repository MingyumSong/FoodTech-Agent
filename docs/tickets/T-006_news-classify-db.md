...
Status: DONE (2026-07-21 — 희정 검수 반영 후 배포·라이브 검증 완료.
검수(`뉴스분류_검수완료.md`) 25건 지적 → ① 프롬프트 v2: 판정 순서 5단계 + reason 강제 + few-shot
(실측 비교는 `prompt-eval-2026-07-21.md` — 명백 무관 기사는 전부 폐기, 경계 케이스만 잔존)
② 위키백과·야후 시세 도메인 차단 ③ 로컬 78건 검수 반영(→59건) ④ 분류 입력 요약 200→300자
⑤ 니치 검색어 3종 naver sim 병행. **AC7 라이브 검증**: railway 배포 → `/jobs/news-refresh` →
Supabase 60건 적재/25건 폐기/도메인 차단 3건 실확인. 운영 1차에서 배치 3개가 빈 응답으로 유실
→ 재트리거로 자기치유 확인 + 배치 1회 재시도 코드 추가.
