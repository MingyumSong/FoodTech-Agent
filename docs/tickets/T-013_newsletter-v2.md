...
Type: FEAT
Status: DOING (2026-08-04 — AC 10/11 충족, check.sh 그린 98개.
  구현: reply_to(배포됨) · B안 재작성(에피2·메인3·디저트) · 국내4:해외1 · 오늘의 분야 줄 ·
  반응 버튼(engagement_events 멱등) · 헤더 아이콘 · 모바일 390px 잘림 없음(스크린샷 검수).
  **잔여: AC2 — 실제 메일에서 답장 주소 확인**(13:00 발송분에 답장해보면 확정). 배포 대기.)
...
- 기사 링크 URL 변형 — 원본 그대로 유지(T-003 클릭 매칭 전제). 이 규칙을 깨려던 T-016(중간 착지
  페이지)은 2026-08-18 접었으므로 **예외 없이 계속 유효하다.**
