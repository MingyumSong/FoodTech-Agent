...
Type: FEAT
Status: DONE (2026-08-04 — AC 7/7 충족, check.sh 그린 135개.
  `app/services/dwell.py`(편 안에서만 간격, 30분 상한, 마지막 클릭 제외) +
  인기분야 탭 "읽은 깊이(추정)" 카드. 운영 데이터 실측(120일): 클릭 135 / 측정가능 63(47%) /
  상한초과 3 / 중앙값 13초 / 튕김24·중간23·정독16 — 세 구간 모두 채워져 임계치 유효.
  7일 기준 화면 스크린샷 검수 완료.)
